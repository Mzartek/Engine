...
void Engine::MovableCamera::setInitialAngle(const GLfloat &t, const GLfloat &p)
{
	...
	GLfloat tmp = cosf(_aphi);
	_vforward->x = tmp * sinf(_atheta);
	_vforward->y = sinf(_aphi);
	_vforward->z = tmp * cosf(_atheta);
...
glm::vec3 Engine::MovableCamera::getForwardVector(void) const
{
	return *_vforward;
}

glm::vec3 Engine::MovableCamera::getLeftVector(void) const
{
	return *_vleft;
}
