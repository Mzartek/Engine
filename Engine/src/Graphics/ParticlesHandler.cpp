...
Engine::Graphics::ParticlesHandler::~ParticlesHandler(void)
{
	glDeleteTransformFeedbacks(1, &_idTFO);
	glDeleteVertexArrays(1, &_idVAO);
}

void Engine::Graphics::ParticlesHandler::loadTexture(const GLchar *path) const
{
	_colorTexture->loadFromFile(path);
}

void Engine::Graphics::ParticlesHandler::setParticles(const Particle *particles, GLsizei numParticles)
{
...
const glm::vec3 &Engine::Graphics::ParticlesHandler::getPosition(void) const
{
	return _position.position;
}

void Engine::Graphics::ParticlesHandler::updateParticles(void)
{
...
void Engine::Graphics::ParticlesHandler::updateParticles(const std::shared_ptr<DepthMap> &depthMap, const std::shared_ptr<Camera> &cam)
{
	glEnable(GL_RASTERIZER_DISCARD);
...
void Engine::Graphics::ParticlesHandler::display(const std::shared_ptr<GBuffer> &gbuf, const std::shared_ptr<Camera> &cam)
{
