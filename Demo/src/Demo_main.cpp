...
void Demo::display(GLfloat state)
{
	...
	if (_step == 0) fireflies_particles->display(gBuffer, camera);
...
{
	static GLfloat rotation[] = { glm::pi<GLfloat>() / 1.25f, glm::pi<GLfloat>() };
	static glm::mat4 movement[2];
	...
	case 0:
		...
		movement[0] = glm::translate(tmp[0]);
		movement[0] *= glm::rotate(rotation[0], glm::vec3(0, 1, 0));
		movement[0] *= glm::translate(glm::vec3(30, -5, 0));
		rotation[0] += 0.002f;

		movement[1] = glm::translate(tmp[1]);
		movement[1] *= glm::rotate(rotation[1], glm::vec3(0, 1, 0));
		movement[1] *= glm::translate(glm::vec3(15, -5, 0));
    rotation[1] -= 0.010f;

    camera->setPositionAndTarget(glm::vec3(movement[0][3]), tmp[0]);
		firefliesEffect->setPosition(glm::vec3(movement[1][3]));

		firefliesEffect->updateParticles();
