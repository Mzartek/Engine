...
void RainEffect::init(const std::shared_ptr<glm::vec3> &position, GLuint numParticles) const
{
	std::vector<Engine::Particle> rainParticles(numParticles);
	for (unsigned int i = 0; i < numParticles; i++)
	{
		rainParticles[i].life = (GLfloat)(rand() % 100);
		rainParticles[i].position = std::shared_ptr<glm::vec3>(new glm::vec3(position->x + (rand() % 200 - 100), position->y + (100 - rainParticles[i].life), position->z + (rand() % 200 - 100)));
		rainParticles[i].direction = std::shared_ptr<glm::vec3>(new glm::vec3(0, -1, 0));
